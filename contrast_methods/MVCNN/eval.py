...
from contrast_methods.MVCNN.mvcnn_model import MVCNNModel
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Extracting Features"):
            ...
            for k in k_list:
                ...
                sub_views = {kk: batch['views'][kk] for kk in sub_keys}
                x_sub = views_dict_to_tensor(sub_views)
                query_feat = model(x_sub, return_projector=False)
...
def main():
    setup_logging()
    parser = argparse.ArgumentParser(description='MVCNN检索评估脚本')
    ...
    checkpoint_path = args.checkpoint or find_latest_checkpoint(os.path.join(ROOT_DIR, 'Checkpoints/MVCNN'))
