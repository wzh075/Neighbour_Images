...
from contrast_methods.MoCoV2.moco_model import MoCoV2Model
...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default=os.path.join(ROOT_DIR, 'Log/MoCoV2'))
    parser.add_argument('--checkpoint_dir', type=str, default=os.path.join(ROOT_DIR, 'Checkpoints/MoCoV2'))
    ...
    model = MoCoV2Model(queue_size=args.queue_size, feature_dim=128, momentum=args.momentum, temperature=args.temperature)
    if device.type == 'cuda':
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in tqdm(dataloader, desc=f"Epoch {epoch}"):
            ...
            views_q = sample_view_subset(batch['views'])
            views_k = sample_view_subset(batch['views'])

            x_q = views_dict_to_tensor(views_q)
            x_k = views_dict_to_tensor(views_k)
    ...
    logging.info("MoCoV2 training finished")
