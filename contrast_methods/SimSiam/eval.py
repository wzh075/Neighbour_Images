...
def extract_features(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc="Extracting Features"):
            ...
            gallery_feat = model.forward_encoder(x_full)
            ...
            for K in K_list:
                ...
                query_feat = model.forward_encoder(x_sub)
...
def main():
    setup_logging()
    parser = argparse.ArgumentParser(description='SimSiam检索评估脚本')
    ...
    checkpoint_path = args.checkpoint or find_latest_checkpoint(os.path.join(ROOT_DIR, 'Checkpoints/SimSiam'))
