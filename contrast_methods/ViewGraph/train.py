...
def main():
    ...
    parser.add_argument('--log_dir', type=str, default=os.path.join(ROOT_DIR, 'Log/ViewGraph'))
    parser.add_argument('--checkpoint_dir', type=str, default=os.path.join(ROOT_DIR, 'Checkpoints/ViewGraph'))
    ...
    student_model = ViewGCNModel(graph_type=args.graph_type)
    teacher_model = ViewGCNModel(graph_type=args.graph_type)
    ...
    for p in raw_student.gcn1.parameters():
        p.requires_grad = True
    for p in raw_student.gcn2.parameters():
        p.requires_grad = True
